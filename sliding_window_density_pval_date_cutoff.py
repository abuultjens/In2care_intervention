...
plt.fill_between(
    range(len(timestamps)), lower_p_threshold, upper_p_threshold, color='gray', alpha=0.2, label='p-value > 0.05 (runs with random treatment site coordinates, n=500)'
)

# Highlight the IQR (Interquartile Range)
plt.fill_between(
    range(len(timestamps)), lower_iqr, upper_iqr, color='gray', alpha=0.4, label='IQR (25%-75%)'
)

# Plot the median of random data
plt.plot(range(len(timestamps)), median_random, color='blue', linestyle='--', linewidth=1.5, label='Median of random data')
...
plt.title(
    ...
    '71-day sliding windows with actual vs randomised treatment coordinates',
    fontsize=16
)
